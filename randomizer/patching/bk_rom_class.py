...
class BK_ROM_CLASS(Generic_Bin_File_Class):
    ...
    def __init__(self, file_path:str):
        ...
        # Variables
        self._append_address:int = INT_CONST.rom_end_index
    ...
    def _create_extracted_files_directory(self):
        ...
        if(not os.path.exists(STR_CONST.extracted_files_dir)):
            os.mkdir(STR_CONST.extracted_files_dir)
    ...
    def _extract_asset_by_pointer(self, pointer_index_start:int, file_name:str):
        ...
        '''
        asset_index_start:int = self._read_bytes_as_int(pointer_index_start, 4) + INT_CONST.asset_table_offset
        asset_index_end:int = self._read_bytes_as_int(pointer_index_start + 0x8, 4) + INT_CONST.asset_table_offset
        file_path:str = f"{STR_CONST.extracted_files_dir}{file_name}{STR_CONST.compressed_bin_extension}"
    ...
    def extract_asset_table_pointers(self):
        ...
        for asset_id in range(
                INT_CONST.asset_table_start_id,
                INT_CONST.asset_table_end_id + 0x1):
            ...
            compressed_obj = COMPRESSION_CLASS(file_name, STR_CONST.compressed)
    ...
    def _obtain_assembly_address_list(self):
        ...
        for asm_name in DICT_CONST.assembly_pointers:
            assembly_dict:dict = DICT_CONST.assembly_pointers[asm_name]
            start_upper_index:int = assembly_dict[STR_CONST.start_upper]
            start_lower_index:int = start_upper_index + assembly_dict[STR_CONST.lower_offset]
            start_address:int = self._get_assembly_address(start_upper_index, start_lower_index)
            end_upper_index:int = assembly_dict[STR_CONST.start_upper] + INT_CONST.end_offset
            end_lower_index:int = end_upper_index + assembly_dict[STR_CONST.lower_offset]
    ...
    def extract_assembly_files(self):
        ...
        for start_address, end_address, asm_name in assembly_address_list:
            data_address:int = self._file_content.find(
                BYTE_CONST.bk_compressed_file_header + b"\x00",
                start_address + 1,
                end_address)
    ...
    def _adjust_asset_pointer_table(self,
        ...
        pointer_start_address:int = self._append_address - INT_CONST.asset_table_offset
        self._write_bytes_from_int(pointer_index_start, pointer_start_address, 4)
        pointer_end_address:int = end_address - INT_CONST.asset_table_offset
    ...
    def append_asset_table_pointers(self):
        ...
        self._append_address:int = INT_CONST.rom_end_index
        bin_files_list = os.listdir(STR_CONST.extracted_files_dir)
        for asset_id in range(
                INT_CONST.asset_table_start_id,
                INT_CONST.asset_table_end_id + 0x1):
            pointer_index_start:int = INT_CONST.asset_table_start_index + asset_id * INT_CONST.asset_table_interval
    ...
    def _calculate_and_set_assembly_pointers(self, asm_name:str, start_address:int, new_file_length:int):
        ...
        Functionally works, Game breaks :(
        '''
        start_upper_index:int = DICT_CONST.assembly_pointers[asm_name][STR_CONST.start_upper]
        start_lower_index:int = start_upper_index + DICT_CONST.assembly_pointers[asm_name][STR_CONST.lower_offset]
        ...
        end_upper_index:int = DICT_CONST.assembly_pointers[asm_name][STR_CONST.start_upper]
        end_lower_index:int = end_upper_index + DICT_CONST.assembly_pointers[asm_name][STR_CONST.lower_offset]
    ...
    def insert_assembly_files(self):
        ...
        for start_address, end_address, code_file_name, data_file_name, asm_name in self._assembly_file_list:
            # Compress Files
            compressed_code_obj = COMPRESSION_CLASS(code_file_name, STR_CONST.decompressed)
            compressed_code_file_path = compressed_code_obj.compress_assembly_file()
            compressed_data_obj = COMPRESSION_CLASS(data_file_name, STR_CONST.decompressed)
            compressed_data_file_path:int = compressed_data_obj.compress_assembly_file()
            # Add Compressed Files Together
            new_file_path:str = STR_CONST.extracted_files_dir + asm_name + STR_CONST.compressed_bin_extension
    ...
    def calculate_new_crc(self):
        ...
        t1 = t2 = t3 = t4 = t5 = t6 = INT_CONST.cic
        for check_index in range(
                INT_CONST.check_rom_start_index,
                INT_CONST.check_rom_end_index,
                INT_CONST.check_rom_interval):
        ...
        self._write_bytes_from_int(INT_CONST.crc1_index_start, crc1, 4)
        self._write_bytes_from_int(INT_CONST.crc2_index_start, crc2, 4)
    ...
    def clear_extracted_files_dir(self, filter:str):
        ...
        for file_name in bin_files_list:
            if(file_name.endswith(filter)):
                os.remove(os.path.join(STR_CONST.extracted_files_dir, file_name))
...
if __name__ == '__main__':
    ...
    bk_rom.clear_extracted_files_dir(STR_CONST.bin_extension)